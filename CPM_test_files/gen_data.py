...
def read_mat(landmark_list, image_list):
	"""
	...
	# key_point_list = []
	limits = np.transpose(landmark_list,(0,2,1))
	...
	return landmark_list, center_point_list, scale_list
...
class MONKEY_DATA(torch.utils.data.Dataset):
	def __init__(self, landmark_list, path, stride, transformer=None):
		self.image_list = read_dataset(path)
		self.landmark_list = landmark_list
		self.key_point_list, self.center_point_list, self.scale_list = read_mat(self.landmark_list, self.image_list)
		self.stride = stride
